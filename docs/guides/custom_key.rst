...
If none of the Key solutions provided by Cosmos SDK or the community are able to meet your requirements, 
...
Is the private key accessible by developer?
    * YES: Subclass :class:`cosmos_sdk.key.raw.RawKey`
    * NO: Subclass :class:`cosmos_sdk.key.key.Key`

Can the signing agent sign arbitrary data payloads AND use ECDSA on Secp256k1?
    * YES: Override :meth:`Key.sign()<cosmos_sdk.key.key.Key.sign>`
    * NO: Override :meth:`Key.create_signature()<cosmos_sdk.key.key.Key.create_signature>`
...
Can you determine the public key in advance, and is it static?
    ...
    * NO: Override :meth:`acc_address<cosmos_sdk.key.key.Key.acc_address>`, :meth:`val_address<cosmos_sdk.key.key.Key.val_address>`, :meth:`acc_pubkey<cosmos_sdk.key.key.Key.acc_pubkey>`, :meth:`val_pubkey<cosmos_sdk.key.key.Key.val_pubkey>` properties.
...
* External signing
    ...
    **Scenario:** The transaction signing is to be performed outside the Python program running Cosmos SDK,
...
.. code-block:: python
    ...
    from cosmos_sdk.key.raw import RawKey
